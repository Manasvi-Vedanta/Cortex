...
# ===========================================
# --- GEMINI IMPORTS (Comment out if using OpenAI) ---
# import google.generativeai as genai
# --- END GEMINI IMPORTS ---

# --- OPENAI IMPORTS (Uncomment if using OpenAI) ---
...
class GenMentorAI:
    ...
    def __init__(self, db_path: str = 'genmentor.db', api_key: str = None, model_name: str = 'all-mpnet-base-v2'):
        ...
        # --- GEMINI INITIALIZATION (Comment out if using OpenAI) ---
        # Configure Gemini API with explicit API key
        # if not api_key:
        #     api_key = "AIzaSyCto9fcsMeZGhoBxfqvO6Swk4bE6Z7Kkvc"  # Your API key
        ...
        # --- OPENAI INITIALIZATION (Uncomment if using OpenAI) ---
        try:
            from config import OPENAI_API_KEY, OPENAI_MODEL
            self.openai_client = OpenAI(api_key=OPENAI_API_KEY, timeout=60.0)
            self.openai_model = OPENAI_MODEL  # e.g., "gpt-5.2"
            self.llm_provider = "openai"
            self.llm_model = True  # Flag to indicate LLM is available
            self.llm_model_flash = True  # Fallback uses same client
            print(f"✅ OpenAI API configured successfully! Model: {OPENAI_MODEL}")
        except Exception as e:
    ...
    def identify_skill_gap(self, goal_string: str, user_current_skills: List[str]) -> Dict[str, Any]:
        ...
        # Clear LLM skill cache for fresh results on each request
        self._goal_skill_cache = {}
        ...
        # Step 4.5: Inject modern skills identified by LLM that are missing from ESCO
        # This addresses the gap where ESCO doesn't include modern technologies like Kotlin, Flutter, etc.
        llm_skill_priorities = self._get_goal_aware_skill_priorities(goal_string)
        relevant_skills = self._inject_modern_skills(relevant_skills, llm_skill_priorities, goal_string)
    ...
    # Cache for goal-aware skill priorities to avoid repeated LLM calls
    _goal_skill_cache = {}
    ...
    def _get_goal_aware_skill_priorities(self, goal_string: str) -> dict:
        ...
        Results are cached to avoid repeated LLM calls for the same goal.
        """
        goal_lower = goal_string.lower()
        
        # Check cache first
        if goal_lower in self._goal_skill_cache:
            return self._goal_skill_cache[goal_lower]
        
        # Try LLM-based skill identification first
        llm_skills = self._llm_identify_relevant_skills(goal_string)
        if llm_skills:
            self._goal_skill_cache[goal_lower] = llm_skills
            return llm_skills
        ...
        domain_skill_mappings = {
            # Mobile Development
            'android': {
                'kotlin': 10, 'java': 9, 'android': 10, 'android sdk': 9, 'mobile': 9,
                'gradle': 8, 'jetpack compose': 9, 'xml': 7, 'firebase': 8, 'room database': 8,
                'retrofit': 8, 'coroutines': 8, 'mvvm': 8, 'rest api': 8, 'git': 7,
                'object-oriented': 8, 'programming': 9, 'software development': 8,
                'testing': 7, 'debugging': 7, 'ui': 7, 'material design': 7
            },
            'ios': {
                'swift': 10, 'swiftui': 9, 'objective-c': 7, 'xcode': 9, 'ios': 10, 'mobile': 9,
                'cocoa': 8, 'uikit': 8, 'core data': 7, 'combine': 8, 'git': 7, 'programming': 9
            },
            'mobile app': {
                'mobile': 10, 'android': 9, 'ios': 9, 'kotlin': 9, 'swift': 9,
                'java': 8, 'flutter': 9, 'dart': 8, 'react native': 8, 'programming': 9
            },
            # Web Development
            'web developer': {
        ...
        for domain, skills in domain_skill_mappings.items():
            if domain in goal_lower:
                ...
                self._goal_skill_cache[goal_lower] = skills
                return skills
        
        # Default: return empty dict, no specific boosting
        self._goal_skill_cache[goal_lower] = {}
        return {}
...
List ONLY technical skills, not soft skills. Be specific to the career mentioned."""
        ...
        try:
            # --- GEMINI LLM CALL (Comment out if using OpenAI) ---
            # response = self.llm_model.generate_content(prompt)
            # response_text = response.text if response and response.text else ""
            # --- END GEMINI LLM CALL ---
            
            # --- OPENAI LLM CALL (Uncomment if using OpenAI) ---
    ...
    def _inject_modern_skills(self, existing_skills: List[tuple], llm_priorities: dict, goal_string: str) -> List[tuple]:
        """
        Inject modern skills identified by LLM that are missing from ESCO database.
        
        This addresses the gap where ESCO doesn't include modern technologies like:
        - Kotlin (Android's recommended language since 2019)
        - Flutter, React Native (modern cross-platform frameworks)
        - Jetpack Compose (modern Android UI toolkit)
        - FastAPI, Next.js, etc.
        
        Args:
            existing_skills: Skills already from ESCO occupation-skill relations
            llm_priorities: Dictionary of skill names and their priority scores from LLM
            goal_string: The user's career goal
            
        Returns:
            Enhanced list of skills including modern ones
        """
        if not llm_priorities:
            return existing_skills
        
        # Get existing skill labels (lowercase for comparison)
        existing_labels = set()
        for skill_data in existing_skills:
            if len(skill_data) >= 2:
                existing_labels.add(skill_data[1].lower())
        
        # Find high-priority LLM skills that are missing from ESCO
        modern_skills_to_add = []
        for skill_name, priority_score in llm_priorities.items():
            skill_lower = skill_name.lower()
            
            # Only add skills with high priority (8+) that aren't already included
            if priority_score >= 8:
                # Check if this skill or something similar is already in the list
                is_covered = False
                for existing in existing_labels:
                    # Check for exact match or if one contains the other
                    if (skill_lower == existing or 
                        skill_lower in existing or 
                        existing in skill_lower):
                        is_covered = True
                        break
                
                if not is_covered:
                    # Create a synthetic skill entry for this modern technology
                    # Format: (uri, label, description, relation_type, vote_score, vote_count)
                    synthetic_uri = f"modern://skill/{skill_name.lower().replace(' ', '-')}"
                    description = f"Modern technology skill identified as essential for {goal_string}"
                    
                    modern_skills_to_add.append((
                        synthetic_uri,
                        skill_name.title(),  # Capitalize properly
                        description,
                        'essential',  # Mark as essential since LLM prioritized it
                        0.0,  # No community votes yet
                        0     # No vote count
                    ))
                    print(f"✨ Injecting modern skill: {skill_name.title()} (LLM priority: {priority_score})")
        
        # Combine: put modern skills near the top (after first few ESCO essentials)
        if modern_skills_to_add:
            # Insert modern skills after the first 3 ESCO skills (keep some foundational ones first)
            result = list(existing_skills[:3]) + modern_skills_to_add + list(existing_skills[3:])
            print(f"📦 Added {len(modern_skills_to_add)} modern skills to learning path")
            return result
...
{{"sessions":[{{"session_number":1,"title":"title","objectives":["obj1"],"skills":["exact skill from list"],"estimated_duration_hours":8,"difficulty_level":"beginner","prerequisites":[]}}]}}"""
        ...
        try:
            print("🤖 Generating learning sessions with LLM...")
            ...
            if '429' in error_str or 'quota' in error_str.lower() or 'rate' in error_str.lower():
                print(f"⚠️ LLM rate limited, trying fallback model...")
                
                # --- GEMINI FALLBACK (Comment out if using OpenAI) ---
                ...
                # --- OPENAI FALLBACK (Uncomment if using OpenAI) ---
                # # OpenAI doesn't have a separate fallback model, so just retry once
                try:
                    import time
                    ...
                    response = self.openai_client.chat.completions.create(
                        ...
                        messages=[{"role": "user", "content": prompt}],
                        temperature=0.2,
                        max_tokens=2048  # gpt-3.5-turbo uses max_tokens, not max_completion_tokens
                    )
                    response_text = response.choices[0].message.content.strip() if response.choices else ""
                    if response_text.startswith('```'):
                        lines = response_text.split('\n')
...
"""
    ...
    def _generate_personalized_content(self, topic: str, user_profile: str, user_background: str) -> Dict[str, Any]:
        """Generate personalized learning material using LLM."""
        ...
        try:
            print(f"🤖 Generating personalized content with LLM for {user_profile} level...")
            ...
            print(f"❌ Error generating personalized content with LLM: {e}")
