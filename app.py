...
if QUIZ_AVAILABLE:
    # --- GEMINI QUIZ GENERATOR (Comment out if using OpenAI) ---
    # quiz_generator = QuizGenerator(GEMINI_API_KEY)
    # print("✅ Quiz generator initialized with Gemini")
    # --- END GEMINI QUIZ GENERATOR ---
    
    # --- OPENAI QUIZ GENERATOR (Uncomment if using OpenAI) ---
    quiz_generator = QuizGenerator(OPENAI_API_KEY)
    print("✅ Quiz generator initialized with OpenAI")
    # --- END OPENAI QUIZ GENERATOR ---
else:
    quiz_generator = None
...
def generate_quiz():
    ...
    try:
        ...
        if 'error' in quiz:
            error_details = quiz.get('error_details', 'Unknown error')
            return jsonify({'error': quiz['error'], 'details': error_details}), 500
